...
export default function Leaderboard({
  ...
  const rankIcon = (rank: number) => {
    ...
    return (
      <span className="text-sm font-black text-slate-600 w-6 text-center">
        {rank}
      </span>
    )
  }
              ...
              return (
                <div key={entry.id} className="flex items-center gap-2.5 group">
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`text-[11px] font-black truncate max-w-[90px] transition-colors ${
    ...
    <div className="w-full space-y-4">
                ...
                <div className="h-3 bg-slate-950 rounded-full border border-white/5 overflow-hidden">
                  <div
                    className={`h-full transition-all duration-1000 ${
                      team.includes('Azul') ? 'bg-blue-600' : 'bg-red-600'
                    }`}
                    style={{
                      width: `${Math.min(
                        (score / (Math.max(...Object.values(teamScores)) || 1)) * 100,
                        100
                      )}%`,
                    }}
                  />
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-2xl border transition-all ${
                  rankBg(rank)
                } ${isMe ? 'ring-2 ring-blue-500 scale-[1.02]' : ''}`}
              >
                ...
                />
                <span
                  className={`flex-1 font-black truncate text-sm ${
                    isMe ? 'text-blue-400' : 'text-white'
                  } ${entry.is_eliminated ? 'text-slate-600 line-through' : ''}`}
                >
                  {entry.nickname}
                  {isMe && (
                    <span className="ml-2 text-[10px] font-black text-blue-400 uppercase tracking-widest bg-blue-500/10 px-2 py-0.5 rounded-full">
                      Tú
                    </span>
                  )}
                  {entry.is_eliminated && (
      ...
      {myEntry && myRank > (compact ? 5 : 15) && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-2xl border bg-blue-500/10 border-blue-500/20 ring-2 ring-blue-500 mt-2">
          <span className="text-xs font-black text-blue-400 w-6 text-center">
            {myRank}
          </span>
          <span className="flex-1 font-black text-blue-400 truncate text-sm">
            {myEntry.nickname} (Tú)
          </span>
          <span className="font-black text-sm text-blue-300 tabular-nums">
